...
```{r, include=T,warning=F,echo=FALSE, message=FALSE}
...
library(gridExtra) #Display options
library(corrplot)  #Correlation identifier
library(pROC)      #AUROC curve package
library(glmnet)    #Logistic Regression package
library(ROCR)      #AUROC curve package
library(caret)     #Powerful predictive analytics package
...
```{r, include=F, warning=F, echo=T}


head(dat.train) #Visualize first six rows of the data

dim(dat.train)  #Check dimension

str(dat.train) #Data Structure

table(is.na(dat.train)) #No Missing Values

summary(dat.train) #Data Summary
...
significant variables that will be useful for modeling purposes. We will be using 
histograms to quickly visualize the distribution of different variables
...
distributions(heavily skewed) and over 90% of observations are 0's therefore they 
...
```{r, include=T,warning=F, echo=T,fig.width=14, fig.height=12}
...
```
We notice that distributions of these categorical variables don't display any 
...
Let's perform the same task for categorical variables. We will be using boxplots 
...
In other words, a response closer to 1 indicates higher chance of making over $50,000, while a response closer to 0 indicates a higher chance of making less than 50,000. 
Therefore, a threshold of 0.5 is used to determine whether an individual is predicted to make more than $50,000 annually or not. 
A confusion matrix and a ROC curve are presented to evaluate how well the model predicts income.
...
```
The model produced an accuracy of about 83.5%
Logistic Regression Inference: The model gives higher accuracy on unseen data when it has all the predictors included.
The model's accuracy decreases when some of the predictors are removed.
